...
NOT_BACKTESTED = {
    # 여기 없는 기준은 화면에서 «가격 기록만으로는 못 재요» 라는 뭉뚱그린
    # 문구로 떨어진다. 기준마다 못 재는 이유가 다르므로 전부 적어 둔다.
    "bigcap": "예전 시가총액 순위 기록이 없어서 못 재요.",
    "earnings_miss": "예전 실적 발표 기록이 없어서 못 재요.",
    "value_quality": "예전 PER·ROE 기록이 없어서 못 재요.",
    "earnings": "예전 실적 발표 기록이 없어서 못 재요.",
    "growth": "예전 매출·이익 기록이 없어서 못 재요.",
    "dividend": "예전 배당 기록이 없어서 못 재요.",
    "stable": "예전 시가총액·수익성 기록이 없어서 못 재요.",
}

CAVEATS = [
    "지금 목록에 있는 종목만 썼습니다. 그사이 목록에서 나간 회사는 계산에 없어서, 실제보다 좋게 나올 수 있습니다(생존 편향).",
    "수수료·세금·체결 가격 차이는 넣지 않았습니다.",
...
def main() -> int:
    ...
    for key, picker, label in (("momentum", pick_momentum, "잘 오르고 있는 주식"),
        ...
        if res:
            if key == "oversold":
                res["partial"] = "가격 조건만으로 계산했어요. 실제 화면엔 수익성 조건이 하나 더 있어요."
